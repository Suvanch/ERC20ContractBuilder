...
contract ATC is ERC20, Ownable {
    ...
    constructor(
        ...
        //add ATC wallets to whitelistAddresses
...
// Calculate Fee distributions
    ...
    function proposeTransactionFee(uint256 fee) public onlyOwner {
        require(
            fee >= 0 && fee <= 99e16,
            "ATC: transaction fee should be >= 0 and <= 99%"
        );
        require(
            !hasPendingFee[Functions.FEE],
            "ATC: There is a pending fee change already."
        );
        require(
            currentTimelocks[Functions.FEE] == 0,
    ...
    function proposeTransactionFeeOwner(uint256 fee) public onlyOwner {
        require(
            fee >= 0 && fee <= 99e16,
            "ATC: sell transaction fee should be >= 0 and <= 99%"
        );
        require(
            !hasPendingFee[Functions.FEE_OWNER],
            "ATC: There is a pending owner fee change already."
        );
        require(
            currentTimelocks[Functions.FEE_OWNER] == 0,
            "Current Timelock is already initialized with a value"
    ...
    function proposeFeeDistribution(
        ...
        require(
            ...
            "ATC: The sum of distribuition should be 100%"
        );
        require(
            !hasPendingFee[Functions.FEE_DIST],
            "ATC: There is a pending dsitribution fee change already."
        );
        require(
            currentTimelocks[Functions.FEE_DIST] == 0,
            "Current Timelock is already initialized with a value"
