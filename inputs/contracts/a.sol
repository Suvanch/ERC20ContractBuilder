...
contract a is ERC20, Ownable {
    ...
    constructor(
        ...
        //add a wallets to whitelistAddresses
...
// Calculate Fee distributions
    ...
    function proposeTransactionFee(uint256 fee) public onlyOwner {
        require(
            fee >= 0 && fee <= 15e16,
            "a: transaction fee should be >= 0 and <= 15%"
        );
        require(
            !hasPendingFee[Functions.FEE],
            "a: There is a pending fee change already."
        );
        require(
            currentTimelocks[Functions.FEE] == 0,
    ...
    function proposeTransactionFeeOwner(uint256 fee) public onlyOwner {
        require(
            fee >= 0 && fee <= 15e16,
            "a: sell transaction fee should be >= 0 and <= 15%"
        );
        require(
            !hasPendingFee[Functions.FEE_OWNER],
            "a: There is a pending owner fee change already."
        );
        require(
            currentTimelocks[Functions.FEE_OWNER] == 0,
            "Current Timelock is already initialized with a value"
    ...
    function proposeFeeDistribution(
        ...
        require(
            ...
            "a: The sum of distribuition should be 100%"
        );
        require(
            !hasPendingFee[Functions.FEE_DIST],
            "a: There is a pending dsitribution fee change already."
        );
        require(
            currentTimelocks[Functions.FEE_DIST] == 0,
            "Current Timelock is already initialized with a value"
	...
	require(
		...
		"a: DevelopmentAddress cannot be zero address"
	);
	...
	require(
		...
		"a: MarketingAddress cannot be zero address"
	);
	...
	require(
		...
		"a: RewardsAddress cannot be zero address"
	);
	...
	require(
		...
		"a: LiquidityAddress cannot be zero address"
	);
